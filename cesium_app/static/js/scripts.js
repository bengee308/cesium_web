function draw_charts_and_plots(prediction_entry_key, source_fname){
    ...
    $.get("/load_source_data/" + String(prediction_entry_key) + "/" + String(source_fname), function(data){
        ...
        if("freq1_freq" in features_dict){
            ...
            var folded_ts_data_plot_options = {
                title: "Period-folded time series data (Period = "+String(period).substring(0,7) + ")",
                hAxis: {title:"phase"},
                vAxis:{title:"magnitude", direction:-1},
                legend: 'none',
                explorer: {actions:['dragToZoom','rightClickToReset']},
                intervals: { style: 'bars', barWidth:0.05,
                             lineWidth:0.6, color:"#b20000",
                             fillOpacity: 0.7},
                pointSize:5,
                lineWidth:0, dataOpacity: 0.7}
